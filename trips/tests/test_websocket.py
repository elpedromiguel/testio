...
@database_sync_to_async
def create_trip(
    pick_up_address='La Pastora',
    drop_off_address='Altamira',
    status='REQUESTED',
    rider=None,
    driver=None
):
...
class TestWebSocket:
    ...
    async def test_join_driver_pool(self, settings):
        ...
        message = {
            'type': 'echo.message',
            'data': 'This is a test message for joining.',
        }
    ...
    async def test_request_trip(self, settings):
        ...
        await communicator.send_json_to({
            'type': 'create.trip',
            'data': {
                'pick_up_address': 'Altamira',
                'drop_off_address': 'La Pastora',
                'rider': user.id,
            },
        })
        response = await communicator.receive_json_from()
        ...
        assert response_data['pick_up_address'] == 'Altamira'
        assert response_data['drop_off_address'] == 'La Pastora'
    ...
    async def test_create_trip_group(self, settings):
        ...
        message = {
            'type': 'echo.message',
            'data': 'This is a test message. Test 8',
        }
        channel_layer = get_channel_layer()
